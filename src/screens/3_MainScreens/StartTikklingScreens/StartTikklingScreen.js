...
// import {USER_AGENT, Config.BASE_URL} from '@env';
  ...
  async function get_user_info() {
    try {
      ...
      const response = await axios.get(
        `https://${Config.Config.BASE_URL}/dev/get_user_info`,
        {
          headers: {
    ...
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        const response = await axios.post(
          `https://${Config.BASE_URL}/dev/post_tikkling_create`,
          {
  ...
  async function put_user_address() {
    try {
      ...
      const response = await axios.put(
        `https://${Config.BASE_URL}/dev/put_user_address`,
        {
          zonecode: zonecode,
          address: address,
